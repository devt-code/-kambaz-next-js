...
export default function QuizEditor() {
  ...
  async function save(
    ...
    try {
      ...
      if (redirect === "details") router.push("../" + qid);
      if (redirect === "list") router.push("../");
      return saved;
    } catch (e: any) {
  ...
  async function publishAndExit() {
    ...
    setError(null);
    try {
      const saved = await api.updateQuiz(qid, quiz);
      setQuiz(saved);
                        ...
                        <div className="border rounded p-3">
                          <Row className="g-2">
                            ...
                            <Col xs={12} md={6}>
                              ...
                              <Form.Control
                                ...
                                value={
                                  quiz.availableUntil
                                    ? quiz.availableUntil.slice(0, 16)
                                    : ""
                                }
                                onChange={(e) =>
                                  setQuiz({
                          ...
                          <Form.Group className="mt-2">
                            ...
                            <Form.Select
                              ...
                              onChange={(e) =>
                                setAssignScope(
                                  e.currentTarget.value as "EVERYONE" | "SOME"
                                )
                              }
                            >
                  ...
                  <div className="d-flex align-items-center gap-2 mb-2" key={i}>
                    <Form.Control
                      ...
                      onChange={(e) =>
                        setLocal((prev) => ({
                          ...prev,
                          acceptableAnswers: (prev.acceptableAnswers || []).map(
                            (a, ii) => (ii === i ? e.target.value : a)
                          ),
                        }))
                      }
                    />
