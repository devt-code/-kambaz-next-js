...
export default function QuizDetails() {
  ...
  useEffect(() => {
    (async () => {
      try {
        setMe(await userClient.profile());
      } catch {}
      ...
      try {
        setLastAttempt(await api.getLastAttempt(qid));
      } catch {}
    })();
  }, [qid]);
    ...
    <div className="container mt-3">
      <div className="d-flex justify-content-between align-items-center mb-2">
        ...
        <div className="d-flex gap-2">
          ...
          {isFaculty && (
            <Button onClick={() => router.push(`./${qid}/edit`)}>Edit</Button>
          )}
        </div>
      </div>
        ...
        <Card.Body>
          ...
          <ListGroup variant="flush">
            ...
            <ListGroup.Item>
              ...
              <strong>{quiz.oneQuestionAtATime ? "Yes" : "No"}</strong>
            </ListGroup.Item>
            <ListGroup.Item>
              Webcam Required:{" "}
              <strong>{quiz.webcamRequired ? "Yes" : "No"}</strong>
            </ListGroup.Item>
            <ListGroup.Item>
              Lock After Answering:{" "}
              <strong>{quiz.lockAfterAnswering ? "Yes" : "No"}</strong>
            </ListGroup.Item>
            <ListGroup.Item>
              Availability: <strong>{availabilityLabel(quiz)}</strong>
            </ListGroup.Item>
            <ListGroup.Item>
              Points: <strong>{total}</strong> • Questions:{" "}
