...
export default function Dashboard() {
  ...
  // TEMP DEBUG LOGS FOR END-TO-END VALIDATION
  console.log("[Dashboard] mount: currentUser", currentUser);
  const router = useRouter();
  ...
  const fetchCourses = useCallback(async () => {
    if (!currentUser) return;
    try {
      // Use the session-backed endpoint to fetch the current user's courses.
      // This returns only courses the current user should see (enrolled or created).
      const data = await client.findMyCourses();
      setCourses(Array.isArray(data) ? data : []);
      console.log("[Dashboard] fetched courses:", data);
    } catch (err) {
      console.error("Error fetching courses:", err);
    }
  }, [currentUser]);
  ...
  const fetchEnrollments = useCallback(async () => {
    ...
    try {
      const data = await client.findEnrollmentsForUser(currentUser._id);
      console.log("[Dashboard] raw enrollments data:", data);
      // Only normalize if data is an array
      const normalized = Array.isArray(data)
        ? data.map((item: any) => {
            if (item && Object.prototype.hasOwnProperty.call(item, "course")) {
              const courseId =
                item.course && typeof item.course === "object"
                  ? item.course._id ?? item.course
                  : item.course;
              return { user: item.user ?? currentUser._id, course: courseId };
            }
            if (item && item._id) {
              return { user: currentUser._id, course: item._id };
            }
            return item;
          })
        : [];
      setEnrollments(normalized);
      console.log("[Dashboard] fetched enrollments:", normalized);
    } catch (err) {
      console.error("Error fetching enrollments:", err);
